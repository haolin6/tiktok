# 2026-06-10 挑战赛反馈修补 Spec

## 目标

针对 `/Users/haolin6/Downloads/挑战赛完成度检查.md` 中的评委视角反馈，按 P0 / P1 / P2 梯度修补当前仓库。修补目标不是扩大项目边界，而是把已经接近完成的直播竞拍全栈链路补到更可展示、更可验证、更符合宣讲版要求的状态。

完成后需要达到：

- P0：必补缺口关闭，评委能从代码、页面、测试和文档看到明确证据。
- P1：体验、性能证明和答辩材料更有说服力。
- P2：可选材料补强，降低演示和外部访问风险。
...
- 不新增数据库表，P0 修补应复用现有 `auctions`、`auction_events`、`bids`、`orders`。
- 默认不引入新的运行时依赖；如 P1 fanout 脚本需要 `socket.io-client`，必须在 spec 对应任务中说明并保持最小范围。
- 不改真实支付、真实直播、复杂鉴权、网关、物流等当前边界外能力。
- 不为了测试通过删除或放宽已有状态机、幂等和并发约束。

## 非目标与需讨论项

以下反馈点不建议直接作为本轮代码修补目标，除非用户单独确认：

| 项 | 不建议直接补的原因 | 本轮替代方案 |
|---|---|---|
| 线上 Demo 部署 | 需要部署前端、API、MySQL、Redis、CORS、迁移、密钥和稳定性，成本明显高于本轮修补 | README 写清本地运行和演示视频，补最终验收日志 |
...
- AI 工具使用位置：需求拆解、spec、编码、测试、文档、排错。
- 3-5 个关键 prompt 摘要：不需要贴隐私或完整对话，但要能说明目标、约束、验收命令。
- AI 参与模块：前端、后端、实时协议、Redis/MySQL、测试、文档。
- 人工把控点：状态机、并发一致性、幂等、密钥安全、范围取舍。
- AI 代码贡献率口径：估算范围即可，强调关键决策人工把控。
...
反馈缺口：`docs/project_delivery_summary.md` 里记录过 `npm run test:e2e` 被端口占用阻断，评委会担心最终未重跑。
...
### P0-3 `user.outbid` 端到端落地

反馈缺口：共享协议和技术答辩列了 `user.outbid`，但当前没有服务端 emit，前端也没有监听。
...
### P0-4 `auction.passed` 前端展示补齐

反馈缺口：后端已支持 `auction.passed`，测试也覆盖了部分广播，但用户端没有显式监听和展示。
...
反馈缺口：后端和数据库已有 `extendThresholdSec`、`extendDurationSec`，但发布页没有让评委看到规则配置能力。
...
反馈缺口：宣讲版要求“支持修改未开始竞拍的规则”，当前没有 PATCH/PUT 路由，也没有前端编辑入口。
...
- 编辑页只编辑竞拍规则，不编辑商品标题、图片、介绍；商品编辑不属于本轮 P0。
...
反馈缺口：现有文档中存在“已写入文档但代码/页面尚未完全落地”的风险，例如 `technical_defense.md` 列了 `user.outbid`，`project_delivery_summary.md` 写了延时参数能力，但反馈指出发布页未暴露。
...
- 自动延时：显示新结束时间和延长秒数。
- 成交/流拍：终态结果区域更明确。
- 提示音可选，不作为 P1 必须项；若做，只能在用户点击出价后触发，避免浏览器自动播放限制。
...
### P1-3 轻量 WebSocket fanout 证明

反馈缺口：并发脚本证明了出价一致性，但没有证明多连接广播能力。

优先级说明：

- 若只交付 P0，只能称为核心功能缺口修补完成，不能称为性能反馈已关闭。
- 若目标是关闭挑战赛完成度反馈中的性能证明问题，P1-3 和 P1-4 必须完成。
...
反馈缺口：`docs/concurrency_demo.md` 只有代表性输出，没有评委易读的指标表。

优先级说明：

- 该项与 P1-3 共同关闭“性能/高并发证明不足”反馈。
...
反馈缺口：旧视频链接外部访问权限未知，现已替换为重录后的 B 站公开视频链接。
...
- README 增加演示视频访问说明：
  - 当前视频链接。
  - 若外部评委无法访问，需要备用公开链接。
- 如果用户提供备用链接，再更新 README 和 `docs/project_delivery_summary.md`。

验收标准：

- 不伪造公开视频链接。
- 明确需要用户确认视频访问权限。
- 提交前必须由用户确认演示视频外部可访问；当前 README 和交付总结使用 B 站公开视频链接。

## 最终验收门槛

P0 完成后必须通过并记录到 `docs/final_acceptance_log.md`：
...
- 用户必须确认演示视频外部可访问。
- README 和交付总结必须使用用户确认的视频链接，不能保留旧链接或待补占位。

## 审核标准

两个审核 agent 均需给出 `PASS` 才算 spec 通过：
...
若任一 agent 给出 `REVISION_REQUIRED`，必须修改本 spec，并再次发送给两个 agent 复审。
